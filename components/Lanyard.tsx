...
function Band({ maxSpeed = 50, minSpeed = 0, isMobile = false }: BandProps) {
  ...
  const dragLerp = useRef(new THREE.Vector3());
 ...
 useFrame((state: { camera: THREE.Camera; pointer: THREE.Vector2 }, delta: number) => {
    if (dragged && typeof dragged !== 'boolean') {
  vec.set(state.pointer.x, state.pointer.y, 0.55).unproject(state.camera);
